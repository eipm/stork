...
def stork_predict(args):
	...
	if len(args) != 6:
		...
		exit()
	deep_learning_architecture = args[1]
	train_dir = args[2]
	test_path = args[3]
	output = args[4]
	nb_classes = int(args[5])

	if deep_learning_architecture == "v1" or deep_learning_architecture == "V1":
		image_size = 224
	else:
		if deep_learning_architecture == "v3" or deep_learning_architecture == "V3" or deep_learning_architecture == "resv2" or deep_learning_architecture == "inception_resnet2":
			image_size = 299
		else:
	...
	if deep_learning_architecture == "v1" or deep_learning_architecture == "V1":
		...
		if deep_learning_architecture == "v3" or deep_learning_architecture == "V3":
			with slim.arg_scope(inception.inception_v3_arg_scope()):
				logits, _ = inception.inception_v3(processed_images, num_classes=nb_classes, is_training=False)
		else:
			if deep_learning_architecture == "resv2" or deep_learning_architecture == "inception_resnet2":
...
if __name__ == '__main__':
	...
	if len(sys.argv) != 6:
		...
		print("The  fifth argument should be number of classes.")
		exit()
	deep_learning_architecture = sys.argv[1]
